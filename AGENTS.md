...
| What shape does this API take? | `packages/contracts/` |
| What does the database look like? | `database/schema/` — never a migration file |
| What are the design rules? | `docs/ssot/DESIGN.md` and `packages/tokens/` |
...
SHAPE    declare the shapes — packages/contracts/ and database/schema/
...
| `database/schema/*` | at SHAPE, human approves | evolves; changes serialize |
| `database/migrations/*` | **generated by diffing** | permanent — never edited |
| `CODEMAP.md`, OpenAPI, types | a script | regenerated |
...
- Never hand-edit a generated file. `CODEMAP.md`, generated OpenAPI, generated types, and
  anything under a `dist/` or `migrations/` directory.
- Never define the same schema twice. One source; everything else is generated from it.
- Never hand-write a migration. Edit `database/schema/`, then generate and lint the diff.
- Never run an `apply` against a database or a cloud. You may plan and diff. CI applies.
- Never write a raw color or spacing value in a component. Use a token.
- Never add a top-level directory. The sanctioned roots are fixed; adding one needs an ADR.
