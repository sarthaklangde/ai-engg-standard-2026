...
| **Generated once, then permanent** | migrations | frozen at merge | **generated, never authored, never edited** |
...
### The sharper version, under AI authorship

"Generate rather than hand-write" started as an argument about staleness. When an agent writes
most of the code it becomes an argument about correctness:

> **Prefer tools where the dangerous output is derived, not authored.**

An authored migration is a sample from a distribution. A generated one is a function of two
schemas — same input, same output, always. The same reasoning applies to API clients from
OpenAPI, types from the database, and the code map from the manifests. Every artifact you move
from the authored side to the derived side is surface area that hallucination can no longer
reach.
...
| **Declared schema** | `database/schema/*.sql` | agent | **human** | evolves; changes serialize |
| **Migrations** | `database/migrations/*` | **a diffing tool** | via the schema diff | **permanent, never edited** |
| Design tokens | `packages/tokens/src/*` | agent | human | evolves |
| Infrastructure | `infra/*` | agent plans, CI applies | human | permanent |
...
### `database/` — two directories with opposite rules

- `schema/` is hand-written and edited freely. It is the contract.
- `migrations/` is generated by diffing, and frozen the moment it merges. Never hand-write one.
- A rename diffs as drop-then-add. It needs a human and an ADR.
- Only one in-flight ticket may touch `schema/`.

### `infra/` — permanent, and the truth lives elsewhere

- The committed code is a claim. The provider's state is the truth. Drift detection reconciles
  them, and it is a gate.
- Agents plan. CI applies. Enforced with credential scope, not instructions.

### Generated files

`CODEMAP.md`, generated OpenAPI, generated types, and `packages/tokens/dist/` are all generated.
Never hand-edit them. `mise run check` regenerates and fails if the committed copy differs.

Commit generated output rather than ignoring it. Three reasons: a removed field shows up in code
review, an agent can read the current shape without booting a database or a service, and CI
