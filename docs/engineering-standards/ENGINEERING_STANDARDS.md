...
| Root | Property | Who runs it |
|---|---|---|
| `apps/` | has an **entry point** | a server, a browser, a scheduler |
| `packages/` | has **exports** | something else imports it |
| `database/` | applied to a database by CI | nothing imports a migration |
| `infra/` | applied to a cloud by CI | nothing imports it |
| `tooling/` | run by CI | nothing imports it |

The tooling reads this split: the package manager globs `apps/*` and `packages/*` to resolve
workspace dependencies, build order comes from the import graph, and affected-detection walks
it. Change `packages/tokens` and every app that imports it rebuilds. Change `infra/` and nothing
does.

**No other top-level directory, without an ADR.** `tooling/checks/root-layout.mjs` enforces
this. A stray `src/`, `lib/`, `utils/`, or `common/` means someone did not know where code goes,
and that directory grows without limit because nothing is ever *not* a util.

**A package needs two consumers, or a hard boundary reason** — published externally, a different
runtime, security isolation. One consumer means it belongs inside that app. This is the
"extract at the third occurrence" rule applied at the package level.

An app's internal structure is its own business. `src/modules/<feature>/` is a reasonable
default inside a server.
...
| Never hand-write a migration. Edit the declared schema and generate. | **GATE** |
| Never run an infrastructure or schema `apply`. Plan and diff only. | **GATE** |
...
**Nobody writes DDL by hand.** State the schema you want in `database/schema/`; a deterministic
tool computes the change. Full rules in `database/README.md`.

| Rule | Level | Decision |
|---|---:|---|
| Hand-written DDL | **GATE** | Banned. Migrations are generated by diffing the declared schema. |
| Migration immutability | **GATE** | A merged migration is never edited. The tool's checksum file enforces this. |
| Destructive-change lint | **GATE** | Every generated migration is linted. Drops, unsafe `NOT NULL`, locking index creation. |
| Renames | **GATE** | A human and an ADR. A rename diffs as drop-then-add, which is data loss. |
| Serialization | **GATE** | One in-flight ticket may touch `database/schema/`. |
| Who applies | **GATE** | CI only. A service never migrates at startup — replicas race. |
| One writer per table | **GATE** | Enforced with database roles, not documentation. Ownership in an ADR. |
...
**Why generated rather than authored.** An agent-written migration is a sample from a
distribution; it is probably right. A generated one is a function of the old and new schema —
same input, same output, always. This moves the highest-stakes artifact in the system out of the
layer that hallucinates. It also collapses review from forty lines of `ALTER` with a backfill to
a three-line schema diff, which is the difference between reviewing and skimming.

## 12a. Infrastructure

| Rule | Level | Decision |
|---|---:|---|
| Agent access | **GATE** | Agents may run `plan`. Agents may never `apply`. Enforced by credential scope. |
| Drift | **GATE** | Scheduled `plan` with a non-zero exit on changes. Console edits silently invalidate the directory. |
| Platform choices | **GATE** | Every one gets an ADR. The reasoning is invisible in the configuration. |
| State | **GATE** | Remote backend, locked. Never in git — it holds secrets. |
| Environments | **GATE** | Directories, not branches. |

## 12b. Design system

| Rule | Level | Decision |
|---|---:|---|
| One token source | **GATE** | `packages/tokens/src/tokens.json`. Every other form is generated. |
| No raw values | **GATE** | No color or spacing literal outside the token source. Lint enforces it. |
| Accessibility | **GATE** | WCAG AA contrast and a visible focus state on every interactive element. |
| Sharing | Guideline | Share tokens always. Share a primitive when a second app needs the identical one. Never share an app-specific pattern. |

Design rules decay fastest under AI authorship: a model reaches for a hex value because it looks
right, and three sprints later the brand color cannot be changed. See `docs/ssot/DESIGN.md`.
